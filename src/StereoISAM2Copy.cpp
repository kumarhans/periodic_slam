...
void StereoISAM2::initializeFactorGraph(){
    ...
    graph.emplace_shared<BetweenFactor<Pose3>>(X(0), X(1), Pose3(Rot3::Ypr(0,.5,0), Point3(.1,0,0)), noise);
    graph.emplace_shared<BetweenFactor<Pose3>>(X(0), X(2), Pose3(Rot3::Ypr(0,-.5,0), Point3(.1,0,0)), noise);
    initialEstimate.insert(X(1), currPose);
    initialEstimate.insert(X(2), currPose);
    frame += 2;
