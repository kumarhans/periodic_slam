...
SplittingFrames::SplittingFrames(ros::NodeHandle &nodehandle,image_transport::ImageTransport &imagehandle):nh_(nodehandle),it_(imagehandle){
    ...
    // angleHigh = 1000.0;
    // angleLow = -1000.0;
    angleHigh = vector<float> { -.23, .1, 10 };    
    angleLow = vector<float> { -10, -.1, .23 };
    count = 0;
    initializaing = true;
    imageIn = false;
}
...
void SplittingFrames::initializeSubsAndPubs(){
    ...
    Leftsub.subscribe(it_,"/mobile_robot/simulated/camera/left/image_raw", 1);
    Rightsub.subscribe(it_,"/mobile_robot/simulated/camera/right/image_raw", 1);
    ...
    image_pub_left = it_.advertise("/mobile_robot/simulated/camera/left/image_raw/1", 1);
    image_pub_right = it_.advertise("/mobile_robot/simulated/camera/right/image_raw/1", 1);
